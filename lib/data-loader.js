...
// ────────────────────────────────────────
const CATEGORIES = [
  { type: '集客・リーチ拡大',   keywords: ['インサイト祭り', 'インサイト参加', '何人いますか', 'あなたは何派', 'どっち派', '何派ですか', 'リポスト', '拡散'] },
  { type: '専門性・価値提供',   keywords: ['サロン', '経営', 'リピート', '売上', '客単価', '集客', '予約', '施術', '開業', '退職', '給付金', '有給', '労働', 'ノウハウ', '方法', '仕組み'] },
  { type: '共感・エンゲージ',   keywords: ['あります？', 'ありません？', 'ありますか？', 'いません？', 'いますか？', 'あるある', 'わかる', '同じ', '辞めたい', '仕事辞め', 'どう思いますか', '感じたことありません'] },
  { type: '日常・雑談',         keywords: [] }, // デフォルト
];
...
function getMonthData(account, year, month) {
  ...
  // 月の最初の月曜日を W1 の開始日とする
  const firstMonday = new Date(monthStart);
  const dow = firstMonday.getDay(); // 0=日, 1=月...
  const offset = dow === 0 ? 1 : dow === 1 ? 0 : 8 - dow;
  firstMonday.setDate(firstMonday.getDate() + offset);

  const byWeek = {};
  posts.forEach(post => {
    const diffMs = post.date - firstMonday;
    const weekNum = diffMs < 0
      ? 1
      : Math.min(Math.floor(diffMs / (7 * 24 * 3600 * 1000)) + 1, 5);
    const w = `W${weekNum}`;
