...
// 07 今後の運用方針
// ─────────────────────────────────────────────
// MTG方針ボックスを生成（mtg_policyがある場合）
function buildMtgPolicy(account) {
  const mtg = account.mtg_policy;
  if (!mtg) return [];
  ...
  blocks.push(
    lbox([
      `📋 ${mtg.summary}`,
      `📄 参照資料：${account.strategy_doc || '—'}`,
    ], C.primary, C.light, C.gray700),
    sp(80),
  );

  // 課題と差別化
  if (mtg.issues_and_diff?.length) {
    blocks.push(
      p(r('▌ 現状の課題と差別化方針', { bold: true, color: C.accent, size: 20 }), { after: 60 }),
      new Table({
        width: { size: 9360, type: WidthType.DXA }, columnWidths: [400, 8960],
        rows: mtg.issues_and_diff.map((item, i) => new TableRow({ children: [
          td(`${i + 1}`, 400, { fill: C.light, bold: true, color: C.primary, align: AlignmentType.CENTER, size: 18 }),
          td(item, 8960, { size: 17 }),
        ]})),
      }),
      sp(80),
    );
  ...
  if (mtg.policy) {
    blocks.push(
      p(r('▌ 運用方針', { bold: true, color: C.accent, size: 20 }), { after: 60 }),
      lbox([mtg.policy], C.accent, C.gray100, C.gray700),
      sp(80),
    );
  }
  ...
  if (mtg.todos?.length) {
    blocks.push(
      p(r('▌ TODO（合意済みアクション）', { bold: true, color: C.accent, size: 20 }), { after: 60 }),
      new Table({
        width: { size: 9360, type: WidthType.DXA }, columnWidths: [400, 8960],
        rows: mtg.todos.map((item, i) => new TableRow({ children: [
  ...
  if (mtg.alert) {
    blocks.push(
      lbox([`⚠️ ${mtg.alert}`], C.red, 'FEF2F2', C.red),
      sp(80),
    );
  }
...
function generateMonthlyReport(account, monthData, outputPath) {
  ...
  // 当月のKPI目標を monthly_kpi_targets から上書き
  const monthKey = `${monthData.year}-${String(monthData.month).padStart(2,'0')}`;
  if (account.kpi_targets_monthly && account.kpi_targets_monthly[monthKey] != null) {
    account = { ...account, kpi_target: account.kpi_targets_monthly[monthKey] };
  }
