...
function evalComment(type, d, total) {
  ...
  if (type === 'インサイト祭り') {
    if (pct < 10) return '集客投稿が不足（全体の' + pct + '%）。最重要施策として増量を推奨';
    return '集客投稿として機能している。内容のバリエーションを増やす';
  }
  if (type === '雑談・日常') {
    if (pct > 50) return '雑談過多（' + pct + '%）。ゴールと無関係な投稿が多い。比率を下げること推奨';
    return '適切な比率。フォロワーとの関係構築として機能している';
  }
  if (d.avgLikes < 1) return '反応薄い（平均' + d.avgLikes + '件）。切り口・訴求の見直しが必要';
  if (d.avgLikes >= 5) return '高反応（平均' + d.avgLikes + '件）。継続・拡張推奨';
  return '標準的な反応。具体性・エピソードを加えると改善見込み';
}
...
function buildSummary(account, monthData, issues) {
  ...
  if (criticalIssues.length > 0) {
    summaryText += `${criticalIssues.map(i => i.title).join('・')}が主要課題として検出されました。`;
    summaryText += `0いいね率${kpi.zeroLikeRate}%と投稿の訴求設計に改善余地があります。`;
    summaryText += '今月の課題を踏まえ、翌月から運用方針を調整します。';
  } else {
    summaryText += '当月は安定した運用を継続できています。引き続き数値改善に向けた取り組みを続けます。';
  }
  ...
  // 警告ボックス（HIGH課題がある場合）
  if (criticalIssues.length > 0) {
    const warnLines = ['⚠️ 重要課題:'];
    criticalIssues.forEach(i => warnLines.push(`　・[${i.severity}] ${i.title}：${i.detail}`));
    blocks.push(
          ...
          new TableRow({ children: [new TableCell({
            ...
            children: [
              p([
                ...
                r(post.type || '—', { size: 17 }),
                r('　　ターゲット訴求：', { bold: true, size: 17, color: C.gray700 }),
                r('（ディレクターが記入）', { size: 17, color: C.gray400 }),
              ], { after: 40 }),
              p([
                r('📋 所感：', { bold: true, size: 17, color: C.red }),
                r('（この欄にコメントを記入）', { size: 17, color: C.gray400 }),
              ], { after: 0, line: 320 }),
            ],
          })]})
        ],
