...
function checkContentMix(result) {
  ...
  // 集客・リーチ拡大投稿が少ない
  const reachRate = (types['集客・リーチ拡大']?.count || 0) / total;
  if (reachRate < 0.1) {
    result.issues.push({
      severity: 'MED',
      category: 'コンテンツ',
      title:    `集客・リーチ拡大投稿が少ない（全体の${Math.round(reachRate*100)}%）`,
      detail:   'リーチを広げるための投稿が不足しています。新規フォロワー獲得につながる問いかけ型・拡散型の投稿を増やすことを検討してください。',
      evidence: `集客投稿: ${types['集客・リーチ拡大']?.count || 0}件 / 全体: ${total}件`,
    });
  }

  // 日常・雑談比率が高すぎる
  const casualRate = (types['日常・雑談']?.count || 0) / total;
  if (casualRate > 0.5) {
    result.issues.push({
      severity: 'MED',
      category: 'コンテンツ',
      title:    `日常・雑談投稿が過半数（${Math.round(casualRate*100)}%）を占めている`,
      detail:   'エンゲージメントは取りやすいが、ビジネスゴールへの導線が薄くなっています。専門性・価値提供系の比率を高めることを推奨します。',
      evidence: `雑談: ${types['日常・雑談']?.count || 0}件 / 全体: ${total}件`,
    });
  }

  // 専門性・価値提供投稿が少ない
  const tipsRate = (types['専門性・価値提供']?.count || 0) / total;
  if (tipsRate < 0.15 && total > 10) {
    result.issues.push({
      severity: 'MED',
      category: 'コンテンツ',
      title:    `専門性・価値提供投稿が少ない（全体の${Math.round(tipsRate*100)}%）`,
      detail:   '「この人の話には価値がある」という信頼構築に必要な専門性コンテンツが不足しています。',
      evidence: `専門性投稿: ${types['専門性・価値提供']?.count || 0}件 / 全体: ${total}件`,
    });
  }
}
